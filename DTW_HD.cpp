...
#define M 2000 //The number of time series
...
#define Epsilon 0.35*sqrt(T)//Threshold used in RTree RangeSearch
...
int main(){
    ...
    cout<<"DTW_HD Test: "<<DTW_HD(datasets[0],datasets[2])<<endl;
    ...
    int count=0;
    vector<int> DTW_groundtruth_Range=DTW_GroundTruth_Range(datasets[0],datasets);
    ...
    int countDTWKNN=0;
    vector<int> DTW_groundtruth_KNN=DTW_GroundTruth_KNN(datasets[0],datasets);
    ...
    int search_times=0;
    vector<int> candidate=RangeSearch(datasets[0],Epsilon,MyTree.m_root,datasets,search_times);
    int count_PAA=0;
    for(vector<int>::iterator it=candidate.begin();it!=candidate.end();++it){
    ...
    int DTW_KNN=0;
    vector<int> candidateDTW_KNN=DTW_KNNSearch(datasets[0],datasets,MyTree.m_root,DTW_KNN);
    int count_DTW_KNN=0;
    ...
    vector<int> candidate_LSH_REnvelope=REnvelope_GroundTruth(datasets[0],datasets);
    ...
    vector<int> candidateLSH=LSH_query(datasets[0],datasets,hash_functions,hash_value);
    int LSH_count=0;
    ...
    vector<int> candidateLSH_KNN=LSH_KNN(candidateLSH, datasets[0], datasets);
    int LSH_KNN_count=0;
    ...
    vector<int> candidateLSH_KNN_LB=LSH_LB_Pruning(candidateLSH, hash_functions, hash_value,datasets[0], datasets);
    ...
    vector<int> candidateLSH_KNN_Intersection=LSH_Intersection_Pruning(candidateLSH, hash_functions, hash_value,datasets[0], datasets);
