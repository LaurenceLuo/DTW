...
#define constraint 5 //LB_Keogh window, must be smaller than T
...
public:
    ...
    /// Minimal bounding rectangle (1-dimensional)
    struct Rect
    {
        float m_min[BlockNum];                      ///< Min length of bounding box
        float m_max[BlockNum];                      ///< Max length of bounding box
    };
    ...
    float CalcDiff(Rect* a_rect)
    {
        ...
        for(int index=0;index<BlockNum;index++){
            diff+=a_rect->m_max[index] - a_rect->m_min[index];
        }
        return diff;
    }
    ...
    void Insert(const float a_max[BlockNum], const float a_min[BlockNum], const int& a_dataId)
    {
